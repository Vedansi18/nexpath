// Relocated verbatim from src/decision-session/options.ts (pre-B11-cutover static content
// system) so the browser extension keeps a working resolveDecisionContent after sub-11's B11
// cutover retired the CLI's copy in favour of the content-template engine. The CLI never uses
// this file — it fully migrated to decision-session/content-template-engine.ts. This file is
// the browser's only content source (via core/decision/options.ts + core/decision/pinch.ts),
// so it is kept here, out of upstream's way, instead of reviving the deleted CLI file.
...
} from './static-content-beginner.js';
