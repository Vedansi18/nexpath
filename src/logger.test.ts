...
describe('logger', () => {
  ...
  // JSON.stringify returns undefined (rather than throwing) for values it
  // cannot represent at all. The type says Record<string, unknown>, but types
  // are not enforced at a JS call site, so the branch must still hold.
  it('marks the payload failed when stringify yields undefined', async () => {
    const { initLogger, log, LOG_PATH } = await import('./logger.js');
    let written = '';
    vi.spyOn(await import('node:fs'), 'appendFileSync').mockImplementation((_p, data) => {
      if (_p === LOG_PATH) written += String(data);
    ...
    initLogger('auto', 'info');
    const unrepresentable = (() => undefined) as unknown as Record<string, unknown>;
    expect(() => log('error', 'stringify_undefined', unrepresentable)).not.toThrow();

    expect(written).toContain('stringify_undefined');
    expect(written).toContain('diagnostic_serialization_failed');
  });

  it('omits the payload entirely when no data is passed', async () => {
    const { initLogger, log, LOG_PATH } = await import('./logger.js');
    let written = '';
    vi.spyOn(await import('node:fs'), 'appendFileSync').mockImplementation((_p, data) => {
      if (_p === LOG_PATH) written += String(data);
    ...
    expect(written).toContain('no_payload');
    expect(written).not.toContain('diagnostic_serialization_failed');
    expect(written.trimEnd().endsWith('no_payload')).toBe(true);
  });
