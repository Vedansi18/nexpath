...
export const PROMPT_ENHANCEMENT_INTERNAL_VOCABULARY_V1: readonly string[] = deriveVocabulary();

/** One whole-word matcher per identifier, so `problem_statements` is not a hit on its own. */
const MATCHERS: readonly { value: string; pattern: RegExp }[] =
  PROMPT_ENHANCEMENT_INTERNAL_VOCABULARY_V1.map((value) => ({
    value,
