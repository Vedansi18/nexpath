...
describe('§6.1 gate 1 — dual-source resolver + migration marker', () => {
  ...
  // B7: cls 6, B8: cls 7, B2: cls 1 stage transitions, B9: cls 8 role-cluster).
  const MIGRATED_CLASS_RECORDS = [...CLASS2_RECORDS, ...CLASS3_RECORDS, ...CLASS5_RECORDS, ...CLASS6_RECORDS, ...CLASS7_RECORDS, ...CLASS1_RECORDS, ...CLASS8_RECORDS];

  it('the migration marker holds the 6 new §4.E2 signals (A12) + the migrated Group-B classes (B3/B4/B6/B7/B8/B2/B9: cls 2/3/5/6/7/1/8)', () => {
    const migratedClassSignals = MIGRATED_CLASS_RECORDS.map((r) => r.signalType);
    expect(MIGRATED_SIGNALS.size).toBe(NEW_E2.length + migratedClassSignals.length); // 6 + 21 + 11 + 8 + 14 + 20 + 7 + 35 = 122
  ...
  it('resolves a not-yet-migrated signalType to static (un-migrated classes still serve from the static set)', () => {
    // Real signalTypes from classes NOT yet in MIGRATED_SIGNALS (class 9/4) — still static.
    // (SPEC_ACCEPTANCE/CONTEXT_LOSS/PHASE_TRANSITION/IDEA_TO_PRD/USER_VALUE_CHECK each sat here until
    // B4/B6/B7/B2/B9 migrated them — the self-correcting canary. OVER_ENGINEERING_CHECK + CONTRACT_TESTING_GAP
    // are class 9 (B10); OBSERVABILITY is class 4 (B5) — all still static.)
    for (const s of ['ABSENCE_OBSERVABILITY', 'ABSENCE_OVER_ENGINEERING_CHECK', 'ABSENCE_CONTRACT_TESTING_GAP', 'x_unknown']) {
