...
export const ABSENCE_CODING_AGENT_MODE_MISMATCH_RECORD: ContentTemplateRecord = {
  ...
  levelForms: {
    1: form("Before you make any changes, pause — this still looks like planning to me. Walk me through the approach first, and wait for my go-ahead before you start building.", "The lightest step: a pause to agree the approach before any building starts."),
    2: form("Hold on before editing anything — we're still working out what to build. Lay out the approach in a few steps for me, and wait for my go-ahead before you make any changes.", "A light pass: the approach laid out and agreed before any edits."),
    3: form("This still looks like planning, not building — pause before you make any changes. Outline the approach step by step, call out anything risky or unclear, and wait for my go-ahead before you start.", "This still looks like planning — changes shouldn't start before the approach is agreed."),
    4: form("We're still in planning here — before any changes, lay out the approach as a short plan (the steps, the trade-offs, what could go wrong), get my sign-off, and only then start building. Ask me for go-ahead before you make any changes.", "Beyond a quick outline: a short plan with trade-offs, signed off before building."),
    5: form("Before building, capture the plan for this: the approach, the steps, the main risks, and how we'll check it — then get my go-ahead and work through it one step at a time. Ask me for go-ahead before you make any changes.", "A durable plan — approach, steps, risks, checks — agreed before building begins."),
  },
};
...
export const ABSENCE_AGENT_MODE_TOO_RESTRICTED_RECORD: ContentTemplateRecord = {
  ...
  l2SafeguardLine: 'Ask me for go-ahead before you make any changes.',
  levelForms: {
    1: form("It looks like you're in a read-only mode but we're ready to build. Lay out the exact steps you'd take, and I'll switch you to an edit mode to run them — ask me for go-ahead before you make any changes.", "The lightest step: the steps laid out, ready to run once the mode is switched."),
    2: form("We're ready to build, but you're in a read-only/plan mode so you can't edit yet. Write out the full step-by-step plan so it's ready to go, then I'll switch you over — ask me for go-ahead before you make any changes.", "A light pass: a full step-by-step plan, ready for the switch to an edit mode."),
    3: form("You're held in a read-only mode while we're trying to implement. Lay out the complete plan (files to touch, the order, anything risky), and I'll switch you to an edit mode to carry it out — ask me for go-ahead before you make any changes.", "A read-only mode is blocking the build — the plan is ready but the edits can't start yet."),
    4: form("You're in a read-only mode but the work needs edits. Lay out the full plan (the files, the order, the risky parts, how we'll check each step); I'll move you to an edit mode and we'll work through it — ask me for go-ahead before you make any changes.", "Beyond a plan: a checked, ordered plan ready to run once the mode is switched."),
    5: form("Before we switch you loose to build, lay out the full plan and how we'll verify each step; I'll move you to an edit mode and we'll work through it together — ask me for go-ahead before you make any changes.", "A durable, verifiable plan agreed before switching to an edit mode to build."),
  },
};
