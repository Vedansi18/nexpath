...
export async function runLevel(
  ...
  // Bug 2 delivery (gated OFF by default): deliver the selected option + its rendered why-desc
  // to the agent. The telemetry above already captured the pure option, so it stays clean.
  return deliverSelectedPrompt(result as string, descBaseByOption.get(result as string));
}
