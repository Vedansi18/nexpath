...
- [ ] `*.stackblitz.com` host variant — **WHY it's in the manifest/permissions:** Bolt.new is built by StackBlitz and runs on StackBlitz infrastructure, so it is mapped to the SAME `bolt` agent (`agent-hosts.ts`: `hostname.endsWith('stackblitz.com') → 'bolt'`; `bolt` adapter origins + all three manifest blocks list both `bolt.new` and `*.stackblitz.com`). It is NOT a separate agent/integration — it's the second domain of the same product. **STATUS: NOT CONFIRMED LIVE** — whether a standalone `*.stackblitz.com` surface serves the same composer/fetch/message DOM Bolt.new does was never verified (only `bolt.new` was live-tested). Options if revisited: (a) leave as-is (harmless — matches how Bolt's own infra is split), (b) live-verify to close this item, or (c) drop `*.stackblitz.com` to ship only the confirmed `bolt.new` surface. User informed 2026-07-07.
